...
// ADD USER
else if (isset($_POST["Get_Fingerid"]) && !empty($_SESSION["add-finger-id"])) {
    echo "add-id" . $_SESSION["add-finger-id"];
}

// ADD USER - LAST STEPS
else if ($_POST["confirm_id"] == $_SESSION["add-finger-id"]) {
    unset($_SESSION["add-finger-id"]);
    echo "unset(\$_SESSION[\"add-finger-id\"])";
}
